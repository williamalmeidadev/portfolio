...
const navLinks = [
  ...
  { href: "#experiencia", label: "Experiência", icon: (
  ...
  { href: "#tecnologias", label: "Tecnologias", icon: (
...
const Header: React.FC = () => {
  ...
  return (
    <header>
      <nav className="nav-pill">
        {navItems.map((link) => (
          <a
            href={link.href}
            key={link.href}
            className={activeId === link.href.replace("#", "") ? "ativo" : ""}
          >
            {link.label}
          </a>
        ))}
        <button className="nav-toggle" id="navToggle" onClick={() => setOverlay(true)}>
      ...
      <div className={`nav-overlay${overlay ? " aberto" : ""}`} id="navOverlay">
        <button className="nav-close" id="navClose" onClick={() => setOverlay(false)}>&times;</button>
